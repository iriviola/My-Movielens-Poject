...
#############
#First add average ranking term, b_m
b_m <- edx %>%
       group_by(movieId) %>%
...
#######################
#I have to use lambda. and I have to find out which is teh best value
#first define possible lambdas
lambdas <- seq(from=0, to=10, by=0.25)
#Define RMSE function on user + movie effect and repeat for each lambdas defined 
rmses <- sapply(lambdas, function(l){
          ...
          pr_ratings <- validation %>% #Prediction user+movie effect
          left_join(b_m, by = "movieId") %>%
          left_join(b_u, by = "userId") %>%
          mutate(pred = mu + b_m + b_u) %>%
          pull(pred)
          ...
          return(RMSE(validation$rating, pr_ratings)) #RMSE for each prediction each lambda movie+ user effect
          })
#To define the best lambda, plot RMSE vs lambdas
qplot(lambdas, rmses)
# and print minimum RMSE that will give me the best lambda 
min(rmses)
#########################
...
###############################
# Now consider the movie + user + genre effect regularized on lambda
###############################
#first define the new group of lambdas 
lambdas2 <- seq(0, 20, 1)
#Define a new RMSE function on user + movie+ genre effect and repeat for each lambdas defined 
